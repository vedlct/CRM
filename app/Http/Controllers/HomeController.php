...
//     return $newFiles;
    ...
    public function revenueList(Request $request) {
        $query = 'SELECT new_file.new_fileId, new_file.leadId, new_file.fileCount, new_file.userId, new_file.created_at, new_file.revenue, leads.website, leads.contactNumber, users.firstName, users.lastName FROM new_file LEFT JOIN leads ON leads.leadId = new_file.leadId LEFT JOIN users ON users.id = new_file.userId';
